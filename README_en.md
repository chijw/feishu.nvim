...
A Neovim frontend for Feishu resources, using `feishu-cli` as its backend.
...
- Use [`chijw/feishu-cli`](https://github.com/chijw/feishu-cli) for backend operations instead of maintaining a separate Feishu API client in Lua.
...
Login and token management are handled by `feishu-cli`.

### 1. Install feishu-cli

Use a release build of `chijw/feishu-cli`. The only requirement is that `feishu-cli` is executable on your machine.
...
- `:Feishu login` runs `feishu-cli auth login` inside a floating terminal
...
There is no build step. Put the repository on your runtimepath and make sure `feishu-cli` is available.
...
- `:Feishu bitable`
- `:Feishu tasks`
  - compatibility alias that opens the same bitable view
- `:Feishu chats`
...
- resolve and open a resource from a Feishu URL

### Bitable

The bitable view is schema-driven and does not depend on fixed field names.
...
- `Enter` / `i` / `a` / `A` / `I` start editing the current field
...
### Resources that currently rely on fallback views
